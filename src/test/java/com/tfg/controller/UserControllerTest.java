...
import com.tfg.controller.UserController;
	...
	private static String clientSecret;
	 ...
	 @Test
	    public void testMakePayment() throws Exception {
	        ...
	        String approvalUrl = "http://localhost:8080/paypal/confirm";
	        Mockito.when(payPalService.createOrder(any())).thenReturn(orderId);
	        Mockito.when(payPalService.getApprovalUrl(orderId)).thenReturn(approvalUrl);

	        MockHttpServletRequestBuilder requestBuilder = post("/paypal/payment")
	                .param("amount", String.valueOf(amount))
	                .principal(principal);

	        mockMvc.perform(requestBuilder)
	                .andExpect(status().is3xxRedirection())
	                .andExpect(redirectedUrl(approvalUrl));
	    }
