...
import com.tfg.controller.ShoppingCartController;
...
public class ShoppingCartControllerTest {
	...
	public void testAddProduct_ok() throws Exception {
		...
		when(shoppingCartService.incrementProductQuantity(any(),any())).thenReturn(mockShoppingCartEntity);
	...
	public void testAddProduct_ko() throws Exception {
		...
		Long productId = 1L;
		int quantity = 2;
		when(shoppingCartService.incrementProductQuantity(any(), any())).thenThrow(new InvalidStockException(userEmail));
	...
	void testUpdateQuantity_Ok() throws Exception {
		...
		ShoppingCartEntity cart = new ShoppingCartEntity();
		when(shoppingCartService.incrementProductQuantity(productId, 2)).thenReturn(cart);
	...
	void testUpdateQuantity_InvalidStockException() throws Exception {
		...
		int quantity = 2;
		String action = "increment";
		doThrow(InvalidStockException.class).when(shoppingCartService).incrementProductQuantity(productId, 2);
