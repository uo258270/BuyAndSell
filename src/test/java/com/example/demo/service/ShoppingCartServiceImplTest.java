...
public class ShoppingCartServiceImplTest {
	...
	public void init() {
		...
		cartService = new ShoppingCartServiceImpl(cartRepo, userRepo, productRepository);
	}
	...
	void incrementProductQuantity_ValidStock() throws InvalidStockException {
		...
		ShoppingCartEntity updatedCart = cartService.incrementProductQuantity(productId);
	...
	void decrementProductQuantity() throws NullDataException {
		...
		cartService.setCart(cart);
		ShoppingCartEntity updatedCart = cartService.decrementProductQuantity(1L);
	...
	void decQuantity_NullDataExceptionThrown() throws NullDataException {
	    ...
	    Mockito.doThrow(new NullDataException("cannot be decremented")).when(cart).decQuantity(any());

	    cartService.setCart(cart);
	    
	    assertThrows(NullDataException.class, () -> cartService.decrementProductQuantity(productId));
	}
